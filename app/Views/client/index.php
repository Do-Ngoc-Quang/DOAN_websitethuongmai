...
<section id="works" class="works">
  <div class="container">
    <div class="section-heading">
      <h1 class="title wow fadeInDown" data-wow-delay=".3s"><?= esc($title_cate) ?></h1>
      ...
      <div class="modal fade" id="addnew" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
          ...
          <form action="<?php use function PHPSTORM_META\type; echo base_url() ?>category" method="POST">
            <?= csrf_field() ?>
            ...
            <div class="modal-content">
              ...
              <div class="modal-body">
                ...
                <input style="margin-left: 30px" type="text" name="name_category">
              </div>
        ...
        <tbody>
          <?php if (!empty($category) && is_array($category)) : ?>
            ...
            <?php foreach ($category as $category_item) : ?>
              <tr>
                <th scope="row"><?= esc($category_item['id']) ?></th>
                <td style="width: 85%"><?= esc($category_item['name_category']) ?></td>
                <td>
                  <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#edit_<?= esc($category_item['id']) ?>">
                    Chỉnh sửa
                  </button>
                  <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#delete_<?= esc($category_item['id']) ?>">
                    Xoá
                  </button>
                </td>
              </tr>
              ...
              <div class="modal fade" id="edit_<?= esc($category_item['id']) ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                  ...
                  <form action="<?php echo base_url() ?>category/update/<?= esc($category_item['id']) ?>" method="POST">
                    <?= csrf_field() ?>
                    <div class="modal-content">
                      <div class="modal-header">
                      ...
                      <div class="modal-body">
                        <label for="">Tên: </label>
                        <input style="margin-left: 30px" type="text" name="name_category" value="<?= esc($category_item['name_category']) ?>">
                      </div>
                      <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Đóng</button>
              ...
              <div class="modal fade" id="delete_<?= esc($category_item['id']) ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                  ...
                  <form action="<?php   echo base_url('category/delete/'.$category_item['id']) ?>" method="delete">
                    <?= csrf_field() ?>

                    <div class="modal-content">
                      <div class="modal-header">
                        <h1 class="modal-title fs-5" id="exampleModalLabel">Bạn có chắc chắn là xoá category này?</h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                      </div>
                      <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Đóng</button>
                        <button type="submit" class="btn btn-primary">Xoá Category này</button>
                      </div>
                    </div>
                  </form>
