            ...
            <?php if (!empty($contact) && is_array($contact)) : ?>
              <?php foreach ($contact as $contact_item) : ?>
                ...
                <tr class="table-default">
                  ...
                  <td style="color: <?= $contact_item['status'] ? 'green' : 'orange' ?>;"><?= $contact_item['status'] ? 'Đã xử lý' : 'Chưa xử lý' ?></td>
                  <td>
                    <button type="submit" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#modal-delete-<?= esc($contact_item['id']) ?>" <?= $contact_item['status'] ? 'hidden' : '' ?>>
          ...
          <div class="modal fade" id="modal-delete-<?= esc($contact_item['id']) ?>" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
              <div class="modal-content">
                ...
                <form action="<?php echo base_url('admin/contact/handle_status/' . $contact_item['id']) ?>" method="POST">
                  <?= csrf_field('') ?>
                  <!-- Modal body -->
